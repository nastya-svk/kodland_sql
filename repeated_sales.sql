...
select distinct
 ...
 wb.balance,
 r.country,
 --bs.user_id student_id,
 (case
  when wb.balance = 0 then (select max(wt.transaction_date)::date from finance.wallet_transaction wt where wt."type" like '%subtract%' and wt.user_id = bs.user_id)
  when wb.balance = -1 then (select date_add('week', -1, max(wt.transaction_date)::date)::date from finance.wallet_transaction wt where wt."type" like '%subtract%'  and wt.user_id = bs.user_id)
  when wb.balance = 1 then (select date_add('week', 1, max(wt.transaction_date)::date)::date from finance.wallet_transaction wt where wt."type" like '%subtract%'  and wt.user_id = bs.user_id)
  else null
 end
 ) zero_balance
