...
min_frt as (
	select
		...
		scs.case_id ,
		--scs.created_at,
		min_sla as frt --время первого ответа стаффа юзеру/время от создания кейса до первого сообщения стаффа
	from staff_case_sla scs
	join first_staff_message fsm 
    	on scs.case_id = fsm.case_id and fsm.created_at = scs.created_at-- and fsm.staff_id = scs.staff_id
...
select 
	...
	count(distinct case_id) as tasks_frt
